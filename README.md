...
* `/OpenPMU-KTH-doc-main`: Contains the 3D CAD models for the physical PMU enclosures and supplementary documentation.
* The `/MATLAB_Scripts` folder (which contains the IEEE C37.118 benchmarking suite and synthetic waveform generators) includes sensitive information that is actively being used in an ongoing thesis project. Therefore, it has been intentionally excluded from this repository and cannot be made public at this time.

### Network and Security
The PMU is designed to stream data over an encrypted tunnel using an **OpenVPN** server. OpenVPN establishes a secure, point-to-point connection that wraps the IEEE C37.118-2011 measurements in cryptographic protocols, ensuring data integrity and preventing unauthorized interception over the public internet. 

Furthermore, local Uncomplicated Firewall (UFW) rules enforce strict IP whitelisting to safely route the baseline data stream to a wide-area openHistorian concentrator on the Nordic PMU data network, while completely isolating the experimental streams and administrative SSH access to the local KTH subnet.