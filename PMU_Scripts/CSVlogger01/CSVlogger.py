...
def makeCSVFilePath(csvFileTime, csvPathRoot):        
    ...
    csvFileName = csvFileTime.strftime("%Y-%m-%d_%H-%M-01.csv")
    csvFilePath = os.path.join(csvPathRoot, "LSM", csvPathYMD, csvFileName)
...
def deleteOldRecords(filePathRoot, daysToKeep, dateNow):
    ...
    # glob.glob allows to search specifically for our new YYYY-MM-DD folders inside LSM
    for path in glob.glob(os.path.join(filePathRoot, "LSM", "*")):
