...
class Receiver:
    ...
    def __init__(self, ip, port, forward, forwardIP='', forwardPort=0):
        ...
        if self.MCcheck(ip):
            ...
            # BUFFER EXPANSION
            # Force Math Engine to use a 4MB receive buffer for incoming raw Multicast data
            self.socketIn.setsockopt(socket.SOL_SOCKET, socket.SO_RCVBUF, 4194304)
    ...
    def receive(self, timeout=1):
        ...
        try:
            xml, __address = self.socketIn.recvfrom(65536)
        ...
        try:
            ...
            print("Error occurred while parsing xml to dict")
            print(xml)
            print(e)
            return None
        try:
